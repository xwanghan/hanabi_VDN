...
```shell
cd /tmp/hanabi_master
CUDA_LAUNCH_BLOCKING=1 python3 main.py --alg=vdn --cuda=True
...
--vdn_server:
1.  buffer_size = 20000, n_episodes = 1000, train_steps = 1000, evaluate_cycle = 1000  
    buffer_szie过大，内存不足                                                                     -- 如图 kill-1
2.  buffer_size = 5000, n_episodes = 100, train_steps = 100, evaluate_cycle = 1000              -- 如图 s1
3.  epsilon = 0.5, min_epsilon = 0.02, anneal_steps = 5e4                                       -- 如图 s2 120*1000
                                                                                                -- 如图 s3
4.  buffer_size = 5000, n_episodes = 100, train_steps = 500, evaluate_cycle = 5              
    evaluate_cycle=self.args.evaluate_cycle*self.args.train_steps                               -- 如图 s4
5.  buffer_size = 5000, n_episodes = 200, train_steps = 200, evaluate_cycle = 5
    添加win_rate，目前win=episode_reward>5                                                        -- 如图 s5
6.  gamma = 0.99 ---> 0.999                                                                     -- 如图 s6
7.  last_action 改为上一回合动作    target_update_cycle = 2000
    buffer_size = 2000, n_episodes = 200, train_steps = 200, evaluate_cycle = 5                 -- 如图 s7
8.  仅VDN 添加 Adam 52 行
        if args.optimizer == "Adam":
            self.optimizer = torch.optim.Adam(self.eval_parameters, lr=args.lr, eps=1.5e-05)
    last_action 改为上一回合动作    target_update_cycle = 2000
    buffer_size = 2000, n_episodes = 10, train_steps = 10, evaluate_cycle = 100
    optimizer = Adam   lr = 6.25e-5    rnn_hidden_dim = 512        训练失败                      -- 如图 s8  
    
--  map = Hanabi-Small player = 2:
1.  batch_size = 64  buffer_size = 20000     evaluate_cycle = 1000   evaluate_epoch = 256    save_cycle = 10000    
    epsilon = 0.3    min_epsilon = 0.1    anneal_steps = 1e6            win_tag = 0             -- 如图 s-s1                
2.  增加loss图示: (改动 runner.py agent.py vdn.py)
    evaluate_cycle = 1   buffer_size = 1000         target_update_cycle = 100
    rnn_hidden_dim = 64  target_update_cycle = 100  lr = 5e-4                                   -- 如图 s-s2 与 s-2仅有lr不同
3.  rnn_layer = 2: (改动 base_net.py)                                                            -- 如图 s-s3
