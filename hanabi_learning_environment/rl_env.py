...
class HanabiEnv(Environment):
  ...
  """
    ...
    def get_state(self):
        s = []
        obss = self._make_observation_all_players()
        for obs in obss["player_observations"]:
            s = s + obs['vectorized']
        return s
    ...
    def get_obs(self):
        ...
        obss = self._make_observation_all_players()
        for obs in obss["player_observations"]:
            s.append(obs['vectorized'])
        return s

    def get_avail_agent_actions(self, agent_id):
        obss = self._make_observation_all_players()
        a = obss["player_observations"][agent_id]['legal_moves_as_int']
        a_u = np.zeros(self.num_moves()+1)
        for i in a:
            a_u[i] = 1
        return a_u
