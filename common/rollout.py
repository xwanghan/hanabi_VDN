...
class RolloutWorker:
    ...
    def generate_episode(self, episode_num=None, evaluate=False):
        ...
        while not terminated and step < self.episode_limit:
            ...
            state = self.env.get_state()
            actions, avail_actions, actions_onehot, reward = [], [], [], []
            for agent_id in range(self.n_agents):
                if observations['current_player'] == agent_id:
                    # obs[agent_id] = self.env.get_obs()[agent_id]
                    ...
                    last_action = [action_onehot] * self.n_agents
                else:
                    # obs[agent_id] = self.env.get_obs()[agent_id]
                    avail_action = np.zeros(self.n_actions)
                    avail_action[-1] = 1
                    action = self.args.n_actions - 1
                    # generate onehot vector of th action
                    action_onehot = np.zeros(self.args.n_actions)
                    action_onehot[action] = 1
                    actions.append(np.int(action))
            ...
            observations, reward, terminated, info = self.env.step(min(actions))
            ...
            r.append([reward])
            terminate.append([np.int(terminated)])
            padded.append([0.])
        ...
        if evaluate and episode_num == self.args.evaluate_epoch - 1 and self.args.replay_dir != '':
            ...
            self.env.close()
        if episode_reward > self.args.win_tag:
            win_tag = 1
            if evaluate:
                print('win', episode_reward, step)
            '''import pickle
            with open('win0.pkl', 'wb')as f:
                pickle.dump([episode, episode_reward, win_tag, step], f)'''
